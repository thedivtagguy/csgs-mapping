<script>
  ...
  onMount(async () => {
    ...
    if (mobileCheck()) {
      ...
      defaultRange = [30, 45, 60, 75, 90, 105, 120, 135, 150, 165, 180, 195, 210, 225];
    }
    ...
    if (window.mobileCheck()) {
      ...
      svg = d3.select("#my_dataviz").append("svg").attr("width", width).attr("height", height);
    }
    ...
    const ordering = d3.scaleOrdinal().domain([1, 2, 3, 4, 5, 6, 7, 8, 9]).range(defaultRange);
    ...
    const node = svg
      ...
      .append("path")
      .attr("d", function (d, i) {
        return pathGenerator(i, nodeRadius);
      })
    ...
    var simulation = d3
      ...
      .force("collide", d3.forceCollide().strength(0.5).radius(avoidOverlapRadius).iterations(1)); // Force that avoids circle overlapping
    ...
    <div class=" items-center max-w-7xl  mx-auto ">
      <div class="grid md:grid-cols-12 items-center sm:-mx-3">
        <div class="w-full col-span-7 md:col-span-3 ">
          <div
            class="px-8 w-full pb-6 space-y-2 sm:max-w-md lg:max-w-lg md:space-y-2 lg:space-y-8 xl:space-y-2 sm:pr-5 lg:pr-0 md:pb-0"
          >
            ...
            <div class="flex md:flex-col w-full flex-wrap gap-2">
              <p class="text-gray-700 text-sm font-medium py-2">
                This chart shows digital spaces by genre. Use the dropdown to filter by keyword or
                click on a blob to read more
              </p>
              ...
              {#each genres as genre}
                <div class="flex flex-wrap gap-1 justify-center items-center">
                  ...
                  <div class="flex flex-wrap md:hidden">
                    <div style="background-color: {genre.color}; width: 100%;" class="py-1 px-2">
                      {genre.genre}
                    </div>
                  </div>
                </div>
