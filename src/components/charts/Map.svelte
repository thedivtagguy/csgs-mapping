...
<svelte:head>
	...
	<p class="text-gray-700 py-2 pb-2 text-sm font-medium ">
	...
	<div class="flex flex-wrap gap-2 py-2 justify-left items-center">
		{#each Object.keys(legend) as key}
				<p style="background-color: {legend[key]}" class="{key == 'Other' ? 'text-white' : 'text-gray-700'} px-2 text-base py-1 font-regular">{key}</p>
		{/each}
	</div>
